...
class MongoDate {
   ...
   */
  modify(duration, direction = 'forward') {
    ...
    switch (dur.range) {
      case 'day':
      case 'week':
        diff = (dur.range === 'week' ? 7 : 1) * dir * dur.amount
        return new MongoDate(date.setDate(date.getDate() + diff))
      case 'month':
        diff = dir * dur.amount
        return new MongoDate(date.setMonth( date.getMonth() + diff))
    }
  }
  add(duration) {
    return this.modify(duration)
  }
  subtract(duration) {
    return this.modify(duration, 'back')
  }
